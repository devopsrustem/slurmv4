...
Вариант A: Decode только на aihub0003 (tp=8, пока aihub0006 с другим драйвером)
...
python3 -m sglang.launch_server \
  ...
  --disaggregation-bootstrap-port 8998 \
  --disable-cuda-graph
aihub0003 — decode, один узел (tp=8):
...
python3 -m sglang.launch_server \
  ...
  --disaggregation-ib-device mlx5_0,mlx5_1,mlx5_2,mlx5_3,mlx5_4,mlx5_5,mlx5_6,mlx5_7 \
  --max-running-requests 128
Роутер (после того как все три ноды ответили The server is fired up):
