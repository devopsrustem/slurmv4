...
sed -n '455,475p' /app/sglang/sglang-0.5.9/lib/python3.12/site-packages/sglang/srt/layers/communicator.py



[dcbsr_dev@tpgds-aihub0007 ~]$ sed -n '455,475p' /app/sglang/sglang-0.5.9/lib/python3.12/site-packages/sglang/srt/layers/communicator.py
                            hidden_states,
...
[dcbsr_dev@tpgds-aihub0007 ~]$ . /app/sglang/sglang-latest/bin/activate
((sglang-0.5.9) ) [dcbsr_dev@tpgds-aihub0007 ~]$ py-spy dump --pid $(pgrep -f "sglang.launch_server")
Process 602289: python3 -m sglang.launch_server --model-path /app/models/Deepseek-R1/ --trust-remote-code --tp-size 16 --host 10.99.91.59 --port 30001 --dist-init-addr 10.99.91.59:5000 --nnodes 2 --node-rank 0 --mem-fraction-static 0.8 --disaggregation-mode decode --disaggregation-transfer-backend nixl --disaggregation-ib-device mlx5_0,mlx5_1,mlx5_2,mlx5_5,mlx5_6,mlx5_7,mlx5_8,mlx5_11 --max-running-requests 128 --disable-cuda-graph
Python v3.12.12 (/usr/bin/python3.12)

Thread 602289 (idle): "MainThread"
    run (asyncio/runners.py:118)
    run (asyncio/runners.py:195)
    run (uvicorn/server.py:75)
    run (uvicorn/main.py:606)
    launch_server (http_server.py:1966)
    run_server (sglang/launch_server.py:28)
    <module> (sglang/launch_server.py:35)
    _run_code (<frozen runpy>:88)
    _run_module_as_main (<frozen runpy>:198)
Thread 604597 (idle): "Thread-1 (_wait_and_warmup)"
    readinto (socket.py:720)
    _read_status (http/client.py:292)
    begin (http/client.py:331)
    getresponse (http/client.py:1430)
    getresponse (urllib3/connection.py:571)
    _make_request (urllib3/connectionpool.py:534)
    urlopen (urllib3/connectionpool.py:787)
    send (requests/adapters.py:644)
    send (requests/sessions.py:703)
    request (requests/sessions.py:589)
    request (requests/api.py:59)
    post (requests/api.py:115)
    _execute_server_warmup (http_server.py:1788)
    _wait_and_warmup (http_server.py:1830)
    run (threading.py:1012)
    _bootstrap_inner (threading.py:1075)
    _bootstrap (threading.py:1032)
((sglang-0.5.9) ) [dcbsr_dev@tpgds-aihub0007 ~]$ py-spy dump --pid $(pgrep -f "launch_server")
Process 602289: python3 -m sglang.launch_server --model-path /app/models/Deepseek-R1/ --trust-remote-code --tp-size 16 --host 10.99.91.59 --port 30001 --dist-init-addr 10.99.91.59:5000 --nnodes 2 --node-rank 0 --mem-fraction-static 0.8 --disaggregation-mode decode --disaggregation-transfer-backend nixl --disaggregation-ib-device mlx5_0,mlx5_1,mlx5_2,mlx5_5,mlx5_6,mlx5_7,mlx5_8,mlx5_11 --max-running-requests 128 --disable-cuda-graph
Python v3.12.12 (/usr/bin/python3.12)
